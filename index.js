(function() {
    ...
    inputElement.addEventListener('input', function(event) {
        ...
        if (inputValue.length > 0) {
            ...
            if(lastDigit != captchaCode[inputValue.length - 1])
            {
                if(captchaErrors)
                {
                    typeChat("Слишком много неверных цифр.");
                    captchaClose(0, 0);
                    captchaErrors = 0
                    return
                }
            ...
            {
                captchaEntered = captchaCode.join("")
                captchaClose(1, 1)
            }
        }
    });
    ...
    function captchaOpen() {
        ...
        for (var i = 0; i < 5; i ++)
        {
            captchaPath[i] = "public/" + captchaCode[i].toString() + ".png";
        }
        ...
        document['getElementsByClassName']('icon1')[0]['src'] = captchaPath[1];
        document['getElementsByClassName']('icon2')[0]['src'] = captchaPath[2];
        document['getElementsByClassName']('icon3')[0]['src'] = captchaPath[3];
        document['getElementsByClassName']('icon3 test')[0]['src'] = captchaPath[4];
        // document['getElementsByClassName']('captchaDiv')[0]['style']['display'] = 'block';
        // document['getElementsByClassName']('typeDiv')[0]['style']['display'] = 'block';
    ...
    function captchaClose(cType, valid) {
        ...
        captchaData = cValue
        document['getElementsByClassName']('group-parent')[0]['style']['display'] = 'none';
        // document['getElementsByClassName']('captchaDiv')[0]['style']['display'] = 'none';
        // document['getElementsByClassName']('typeDiv')[0]['style']['display'] = 'none';
        ...
        if (cType == 1) {
            ...
            typeChat((isRecord ? "[РЕКОРД] " : "") + "Капча введена " + (valid ? "" : "не") + "верно (" + captchaEntered + '|' + captchaData + ') за ' + captchaTime + "s (первый символ: " + (firstSymbol != 0 ? firstSymbol + "s" : "нет") + (mode == 1 ? timeReact : "") + ")");
        }
    ...
    window.onload = () => {
        ...
        document.addEventListener('keyup', function(_0xb162x8) {
            ...
            if(event['keyCode'] === 27 && (captchaStatus))
            {
                captchaClose(0, 0);
            }
        })
